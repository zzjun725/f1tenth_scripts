...
arcOffsetLongV = 0.0  # 1.2
arcOffsetLongH = 0.0 # 0.15
arcOffsetShortH = 0.0 # 0.5
arcOffsetShortV = 0.0  # 0.1
arcOffsetCurve = 0.0 # 0.5
cubic_k = 0 # 7
arcPointsNum =  0 # 15
LongPointsNum = 60 # 40
ShortPointsNum = 40  # 20
...
half_left_y = np.linspace(v/2, left_certerP[1], int(LongPointsNum/2))
...
half_bottom_x = np.linspace(h/2, bottom_certerP[0], int(ShortPointsNum/2))[::-1]
...
half_left_bottom = np.hstack([np.vstack([half_bottom_x, half_bottom_y]), np.vstack([half_left_x, half_left_y])])  # (2, n)
...
if Draw:
    ...
    wp_file = open(os.path.join(home, 'wp_interp.csv'), 'w')
